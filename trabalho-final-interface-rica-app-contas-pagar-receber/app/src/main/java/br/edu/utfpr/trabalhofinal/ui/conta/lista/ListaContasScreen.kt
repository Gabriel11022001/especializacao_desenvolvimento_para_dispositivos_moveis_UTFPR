            ...
            ListItem(
                ...
                leadingContent = {
                    Icon(
                        ...
                        contentDescription = "",
                        tint = if (conta.tipo.name == TipoContaEnum.DESPESA.name) {
                            Color.Red
                        } else {
                            Color.Green
                        },
                ...
                trailingContent = {
                    Text(
                        text = if (conta.tipo.name == TipoContaEnum.DESPESA.name) {
                            "-R$${ conta.valor }"
                        } else {
                            "$${conta.valor}"
                        },
                        color = if (conta.tipo.name == TipoContaEnum.DESPESA.name) {
                            Color.Red
                        } else {
                            Color.Green
                        }
...
fun Totalizador(
    ...
    Row(
        ...
        Text(
            ...
            textAlign = TextAlign.End,
            text = if (valor.toDouble() < 0) { "-${ valor.formatar() }" } else { valor.formatar() },
            color = if (valor.toDouble() < 0) {
                Color.Red
            } else {
                Color.Green
            }
